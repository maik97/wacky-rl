...
class TensorMemoryDict(UserDict):

    def __init__(self, *args, **kwargs):
        self.stacked = False
        self.maxlen = None
        super(TensorMemoryDict, self).__init__(*args, **kwargs)
    ...
    def __getitem__(self, y):
        ...
        return super(TensorMemoryDict, self).__getitem__(y)
        ...
        if not self.stacked:
            ...
            if not isinstance(value, TensorList):
                e_3 = (f"\n Setting value at key '{key}' failed: Invalid type {type(value)}"
                       f"\n While not stacked, assigned values to keys of {TensorMemoryDict} must be convertable to {list},"
            ...
            raise TypeError(f"\n Setting value at key '{key}' failed:"
                            ...
                            f"\n While stacked, assigned values to keys of {TensorMemoryDict} must be type {th.Tensor}."
                            f"\n Call the clear() method first, if you are trying to store a list of tensors.")

        super(TensorMemoryDict, self).__setitem__(key, value)
    ...
    def clear(self) -> None:
        self.stacked = False
        super(TensorMemoryDict, self).clear()
